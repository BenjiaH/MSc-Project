function [navSolutions, eph] = postNavigation(trackResults, L5dataNew, settings)
...
for channelNr = activeChnList
    ...
    navBitsSamples = trackResults(channelNr).NavBits(subFrameStart(channelNr) - 20 : ...
...
%% Find pseudoranges ======================================================
    [pseudorangesL1, pseudorangesL5] = calculatePseudorangesL1andL5(...
            trackResults, L5dataNew,...
            subFrameStart + settings.navSolPeriod * (currMeasNr-1), ...
            activeChnList, settings);

     % choose what is passed into the navigation solution
     if (settings.navMode == 'L1only')
        navSolutions.channel.rawP(:, currMeasNr) = pseudorangesL1;
     elseif (settings.navMode == 'L5only')
        navSolutions.channel.rawP(:, currMeasNr) = pseudorangesL5;   
     elseif (settings.navMode == 'L1andL5')
        % add iono free combination here 
     end    
%% Find satellites positions and clocks corrections =======================
    [satPositions, satClkCorr] = satpos_akos(transmitTime, [trackResults(activeChnList).PRN],eph);
%% Find receiver position =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        for ii=1:length(activeChnList)
            freqforcal(ii)=trackResults(1,activeChnList(ii)).carrFreq(currMeasNr*1000/(1000/settings.navSolPeriod));
        end
        
        [xyzdt, ...
        navSolutions.channel.el(activeChnList, currMeasNr), ...
        navSolutions.channel.az(activeChnList, currMeasNr), ...
        navSolutions.DOP(:, currMeasNr)] = leastSquarePosVel(satPositions, navSolutions.channel.rawP(activeChnList, currMeasNr)' + satClkCorr * settings.c, ...
                                            freqforcal, c);
        %--- Save results -------------------------------------------------
