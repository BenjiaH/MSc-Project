function [trackResults, channel]= tracking_FLL_2ndOrderUnknownDataL5(fid, channel, settings)
% Performs code and carrier tracking for all channels.
%
%[trackResults, channel] = tracking_FLL_2ndOrderUnknownDataL5(fid, channel, settings)
%
%   Inputs:
...
% The absolute sample in the record of the L5 code start:
...
codePeriods = settings.msToProcess;     % For GPS one L5 code is one ms
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        % Get a vector with the L5 code sampled 1x/chip
%         codeL5 = L5I_codes(channel(channelNr).PRN,:);
        codeL5 = L5Q_codes(channel(channelNr).PRN,:);
        % Then make it possible to do early and late versions
        codeL5 = [codeL5(10230) codeL5 codeL5(1)];
        ...
        deltaFco_k = settings.spectrumInversion*(carrFreq - settings.IF)/115;
...
%             iBasebandSignal = carrSin .* rawSignal;
            ...
            % accumulate 1ms
            I_E = sum(earlyCode  .* iBasebandSignal).*NHcode(accmCount);
            Q_E = sum(earlyCode  .* qBasebandSignal).*NHcode(accmCount);
            I_P = sum(promptCode .* iBasebandSignal).*NHcode(accmCount);
            Q_P = sum(promptCode .* qBasebandSignal).*NHcode(accmCount);
            I_L = sum(lateCode   .* iBasebandSignal).*NHcode(accmCount);
            Q_L = sum(lateCode   .* qBasebandSignal).*NHcode(accmCount);
...
%% Find PLL error and update carrier NCO         
                ...
                carrLockInd = carrLockInd + (dot - carrLockInd)/(1000/coherentAccmNum);
                codeLockInd = codeLockInd + (I2_plus_Q2 - codeLockInd)/(1000/coherentAccmNum);
...
%% Find DLL error and update code NCO -------------------------------------
            ...
            if coherentAccmNum == 1
                % store the sign of the last 20 correlations
                reg(1:19)   = reg(2:20);
                reg(20)      = NavBit;
                % calculate the correlation with the NH code
                NHsum = sum(reg.*NHcode);
                
            end
            
            % check if more than 50 bit edges in a bin
            if (coherentAccmNum == 1) && (abs(NHsum)==20) && (loopCnt>2000)                       
