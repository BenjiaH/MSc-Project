function acqResults = acquisition_weakHEO(fid, settings)
    ...
    for frqBinIndex = 1:numberOfFrqBins
        ...
        for n = 1:numNonCoherentIntegrations
            ...
            for i = 1:numberOfBranches
                ...
                I = real(coherentSums(i, :) );
                Q = imag(coherentSums(i, :) );
                % sum using sqrt
%                 nonCoherentSums(i, :)  = nonCoherentSums(i, :)  + sqrt(I.^2+Q.^2);
                % sum using Robertson Aproximation                
                nonCoherentSums(i, :)  = nonCoherentSums(i, :)  + max([abs(I)+abs(Q)/2;abs(Q)+abs(I)/2],[],1);
            end
        end 
        % find the maximum
        ...
        % record the result
        resultsCoherent(frqBinIndex, :) =  sqrt(abs(coherentSums(branchBinIndex,:)).^2);
        % record the branch
...
%% Look for correlation peaks in the results ==============================
    ...
    acqResults.peakMetric(PRN) = acqResults.SNR(PRN);
    ...
    acqResults.SNR_includingPeak(PRN)= (peakSize - mean(resultsNonCoherent(frequencyBinIndex, :)))/std(resultsNonCoherent(frequencyBinIndex, :));
