function [trackResults, channel]= tracking_FLL_2ndOrderUnknownData_knownBitBranch(fid, channel, settings)
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        carrLockInd = 0;
        NavBit = 1;
        if ((channel(channelNr).codePhase-1)>samplesPerCode/2)
            adjust = 1;
        else
            adjust = 0;
        end
        bitBranch =( 21-channel(channelNr).bitBranch+1)+adjust;
        accmCount = rem(bitBranch,20);
