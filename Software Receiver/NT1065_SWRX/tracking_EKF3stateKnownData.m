function [trackResults, channel]= tracking_EKF3stateKnownData(fid, channel, settings,EKF_track)
...
%% Generate the six standard accumulated values ---------------------------
            ...
            % no data transitions so just switch to 20ms tracking after 5s
            if loopCnt == 5000                          
...
%% Find PLL error and update carrier NCO         
                ...
                if EKF_enable == 1 
                    ...
                    EKF_track=EKF_3stateKnownData(EKF_track,I_P_S,Q_P_S);
                    % wait till 12.5s to allow the EKF to converge and then use updates for the loop 
                    if loopCnt > 12500             
                            % phase update
                            remCarrPhase=remCarrPhase+EKF_track.X_plus(1);
                            % frequency update
                            carrNco = (carrFreq-carrFreqBasis)+(EKF_track.X_plus(2))/2/pi;
                    end
