function acqResults = acquisitionL5_10ms(longSignal, settings)
...
signal2 = longSignal(0.5*samplesPerCode+1 : 10.5*samplesPerCode);
...
% Generate all L5 codes and sample them according to the sampling freq.
...
%% Correlate signals ======================================================   
    ...
    caCodeFreqDom = conj(fft(L5codesTable(PRN, :)));
...
%             Q2      = cosCarr .* signal2(((timeIndex-1)*samplesPerCode)+1:timeIndex*samplesPerCode);
            ...
            I1      = real(sigCarr .* signal1(((timeIndex-1)*samplesPerCode)+1:timeIndex*samplesPerCode));
            Q1      = imag(sigCarr .* signal1(((timeIndex-1)*samplesPerCode)+1:timeIndex*samplesPerCode));
            I2      = real(sigCarr .* signal2(((timeIndex-1)*samplesPerCode)+1:timeIndex*samplesPerCode));
            Q2      = imag(sigCarr .* signal2(((timeIndex-1)*samplesPerCode)+1:timeIndex*samplesPerCode));
...
%             acqRes1 = acqRes1 + ifft(convCodeIQ1);
%             acqRes2 = acqRes2 + ifft(convCodeIQ2);
        end
        
        %--- Check which msec had the greater power and save that, will
        %"blend" 1st and 2nd msec but will correct data bit issues
%         acqRes1=(abs(ifft(acqRes1)) .^ 2);
%         acqRes2=(abs(ifft(acqRes2)) .^ 2);
