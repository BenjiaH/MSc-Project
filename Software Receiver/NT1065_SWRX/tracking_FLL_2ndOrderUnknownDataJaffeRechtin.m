function [trackResults, channel]= tracking_FLL_2ndOrderUnknownDataJaffeRechtin(fid, channel, settings)
...
%                 Ncod = (1/(2*pi*PDIcarr));
                ...
                % Doppler shift
%                 omega_kplus1 = omega_k + PDIcarr*omegaDot_k + (noiseBandwidth*sqrt(2)*deltaFca_k*PDIcarr);
                omega_kplus1 = omega_k + PDIcarr*omegaDot_k + (noiseBandwidth*sqrt(2)*deltaFca_k);
               
                % Rate of change of Doppler shift
%                 omegaDot_kplus1 = omegaDot_k + (noiseBandwidth^2*deltaFca_k*PDIcarr);
                
                omegaDot_kplus1 = omegaDot_k + (noiseBandwidth^2*deltaFca_k);
                % NCO update
                carrNco = omega_kplus1;

                % Store Doppler shift for next iteration
                omega_k = omega_kplus1;

                % Store rate of change of Doppler for next iteration
                omegaDot_k = omegaDot_kplus1;
...
%% Record various measures to show in postprocessing ----------------------
            ...
            trackResults(channelNr).pllDiscr(loopCnt)       = F*(1/(2*pi*PDIcarr));
            trackResults(channelNr).pllDiscrFilt(loopCnt)   = carrNco;
            trackResults(channelNr).carrierDoppler(loopCnt) = omega_k;
