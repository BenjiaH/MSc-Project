function [trackResults, channel]= tracking_EKF3stateUnknownData(fid, channel, settings, EKF_track)
% Performs code and carrier tracking for all channels.
%
%[trackResults, channel] = tracking_EKF3stateUnknownData(fid, channel, settings)
%
%   Inputs:
...
%       acqResults      - acquisition results, required for noise channel 
%   Outputs:
...
trackResults.EKF_X_minus      = zeros(3, settings.msToProcess);
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        EKF_phiMinus_k = 0;
        % inital Doppler shift estimate
        EKF_deltaFcaMinus_k = 0;
        % inital rate of change of Doppler shift estimate
        EKF_deltaFcaDotMinus_k = 0;
...
%% Generate the six standard accumulated values ---------------------------
            ...
            if (coherentAccmNum == 1) && any(bitSyncCnt == 10)                       
                   % record bit sync 0 to 19 
                   bitSync = find(bitSyncCnt == 10)-1;  
                   % increase the coherent accumulations
                   coherentAccmNum = 20;
                   PDIcode = 0.001*coherentAccmNum;
                   PDIcarr = 0.001*coherentAccmNum;
                   % start the PLL
                   % Adjust the PLL gains accordingly ;
                   PLL_enable = 1;
                   Bl_ca = 10;
            ...
            % switch to EKF after 10s
            if loopCnt == 2000
                 EKF_enable = 1;
                 EKF_track.X_plus=zeros(EKF_track.STATES,1);
                 EKF_track.X_plus(3)=deltaFcaDotMinus_kPlus1;
                 EKF_carrNco = carrNco;
                 EKF_phiMinus_k = 0;
                 EKF_deltaFcaMinus_k = deltaFcaMinus_k;
            end
...
%                 else  
                ...
                % use FLL is EKF is not enabled
                if EKF_enable == 1 
                    
                    EKF_track=EKF_3stateUnknownData(EKF_track,I_P_S,Q_P_S);
                                
                            
                    phaseError=EKF_track.X_plus(1);
                    freqError=EKF_track.X_plus(2);
                    % instant update
%                     remCarrPhase=remCarrPhase+phaseError;
%                     carrNco = ((carrFreq-carrFreqBasis)+(freqError)/2/pi);

                    % increment update
                    carrNco = ((carrFreq-carrFreqBasis)+(freqError)/2/pi)+((phaseError - EKF_phiMinus_k)/(2*pi*PDIcarr));

                     % Store phase estimate for next iteration
                    EKF_phiMinus_k = phaseError;

%                  

                    % Store rate of change of Doppler for next iteration                    deltaFcaDotMinus_k = deltaFcaDotMinus_kPlus1;
                    ...
                    % closed loop EKF so zero phase and frequency states
                    EKF_track.X_plus(1)=0;
                    EKF_track.X_plus(2)=0;
                end
...
%% Find DLL error and update code NCO -------------------------------------
                ...
                % increment the accumlation counter
...
%% Record various measures to show in postprocessing ----------------------
            ...
            trackResults(channelNr).pllDiscr(loopCnt)       = EKF_carrNco;
            
            trackResults(channelNr).pllDiscrFilt(loopCnt)   = carrNco;
            trackResults(channelNr).carrierDoppler(loopCnt) = EKF_Doppler;
            ...
            trackResults(channelNr).EKF_X_minus(:,loopCnt)= EKF_track.X_minus;
