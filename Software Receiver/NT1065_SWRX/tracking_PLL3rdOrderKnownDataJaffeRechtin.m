function [trackResults, channel]= tracking_PLL3rdOrderKnownDataJaffeRechtin(fid, channel, settings)
% Performs code and carrier tracking for all channels.
%
%[trackResults, channel] = tracking_PLL3rdOrderKnownData(fid, channel, settings)
%
%   Inputs:
...
%% Find PLL error and update carrier NCO         
                ...
                noiseBandwidth = 1.2 * Bl_ca;    
                % Implement carrier loop discriminator (phase detector)
                Patan = atan2(Q_P_S, I_P_S);
               ...
               phi_kPlus1 = phi_k + (PDIcarr * phiDot_k) + (2 * noiseBandwidth * PDIcarr * deltaPhi_k);
               phiDot_kPlus1 = phiDot_k + (PDIcarr * phiDotDot_k) + (2 * (noiseBandwidth^2) * PDIcarr * deltaPhi_k);
               phiDotDot_kPlus1 = phiDotDot_k + ((noiseBandwidth^3) * PDIcarr * deltaPhi_k);
                
               carrNco = phiDot_kPlus1/(2*pi) + ((2 * noiseBandwidth * PDIcarr * deltaPhi_k)/(2*pi*PDIcarr));
        
               phi_k = phi_kPlus1;
               phiDot_k = phiDot_kPlus1;
               phiDotDot_k = phiDotDot_kPlus1;
