function generateCAcodeComplex(outputFilename, PRN, Doppler, noiseAmp, settings)
...
fidOut_I = fopen([outputFilename '_I.dat'], 'wb');
fidOut_Q = fopen([outputFilename '_Q.dat'], 'wb');
...
%% Generate the carrier frequency to mix the signal to baseband -----------
        ...
        % reduced signal power 3dB every 10s
        if (rem(loopCnt,10000)== 0)
            noiseAmp = noiseAmp/sqrt(2);
        end
        if (rem(loopCnt,20) ==0)
            navBit = navBit.*sign(randn(1));
        end

        % Finally compute the signal to mix the collected data to bandband
        modulatedSig = cos(trigarg(1:blksize)).*promptCode.*navBit + 1i*sin(trigarg(1:blksize)).*promptCode.*navBit;
        ...
        fwrite(fidOut_I, real(dataOut), 'int8');
        fwrite(fidOut_Q, imag(dataOut), 'int8');
